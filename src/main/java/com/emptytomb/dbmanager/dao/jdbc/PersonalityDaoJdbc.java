...
/**
 * The PersonalityDaoJdbc class is a concrete JDBC implementation of the PersonalityDao interface.
 ...
 * @since   2016-05-31
 */
public class PersonalityDaoJdbc implements PersonalityDao {
  private Logger logger = LoggerFactory.getLogger(PersonalityDaoJdbc.class);
  private static PersonalityDaoJdbc instance = null;
  private Connection connection = null;
     
   private PersonalityDaoJdbc() {
	connection = ConnectionFactory.getInstance().getConnection();
  }
  
  /**
   * This method returns the single instance of the PersonalityDaoJdbc object.
   * 
   * @return  the PersonalityDaoJdbc object
   * 
   */
  public static PersonalityDaoJdbc getInstance() {
      if (instance == null) {      
          synchronized (PersonalityDaoJdbc.class) {
              if (instance == null) {
                  instance = new PersonalityDaoJdbc(); 
              }
          }
      }
      return instance;
  ...
  public Personality getPersonality(int id) throws DaoException {
      ...
      String sql = "SELECT * FROM " + Constants.PERSONALITY_TABLE + " WHERE " + Constants.PERSONALITY_ID + " = ?;";
  ...
  public List<Personality> getPersonalities() throws DaoException {
      ...
      String sql = "SELECT * FROM " + Constants.PERSONALITY_TABLE + ";";
      try (PreparedStatement pstmt = connection.prepareStatement(sql);
  ...
  public int addPersonality(Personality personality) throws DaoException {
      int autoIncKey = -1;
	  String sql = "INSERT INTO " + Constants.PERSONALITY_TABLE + "(" +
    		       Constants.PERSONALITY_ORGANIZATION_ID + "," +     
    		       Constants.PERSONALITY_NAME + "," +
    		       Constants.PERSONALITY_TITLE + "," +
 	               Constants.PERSONALITY_BIO + "," +
	               Constants.PERSONALITY_PICTURE + ") " +  "VALUES" +
 	               "(?,?,?,?,?)";
  ...
  public void updatePersonality(Personality personality) throws DaoException {
      String sql = "UPDATE " + Constants.PERSONALITY_TABLE + " SET " +
    		       Constants.PERSONALITY_ORGANIZATION_ID + "=?, " +     
    		       Constants.PERSONALITY_NAME + "=?, " +
    		       Constants.PERSONALITY_TITLE + "=?, " +
 	               Constants.PERSONALITY_BIO + "=?, " +
	               Constants.PERSONALITY_PICTURE + "=? " +  "WHERE " + 
	               Constants.PERSONALITY_ID + "=?;";
  ...
  @Override
   public void deletePersonality(int id) throws DaoException {
      String sql = "DELETE fROM " + Constants.PERSONALITY_TABLE + " WHERE " + Constants.PERSONALITY_ID + " = ?;";
      try (PreparedStatement pstmt = connection.prepareStatement(sql);) {
     	  pstmt.setInt(1, id);
      	  pstmt.execute();
      } catch (SQLException e) {
   ...
   private Personality getPersonalityFromResultSet(ResultSet resultSet) throws SQLException {
       ...
       personality.setPersonalityId(resultSet.getInt(Constants.PERSONALITY_ID));
       personality.setOrganizationId(resultSet.getInt(Constants.PERSONALITY_ORGANIZATION_ID));
       personality.setName(resultSet.getString(Constants.PERSONALITY_NAME));
       personality.setTitle(resultSet.getString(Constants.PERSONALITY_TITLE));
       personality.setBio(resultSet.getString(Constants.PERSONALITY_BIO));
       personality.setPicture(resultSet.getBytes(Constants.PERSONALITY_PICTURE));
       return personality;
   }
}